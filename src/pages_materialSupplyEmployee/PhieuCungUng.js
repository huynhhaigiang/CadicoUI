...
const SupplyPage = () => {
  ...
  const handleEdit = async supply => {
    ...
    try {
      // Load data for DeXuatVatTu first
      await fetchDeXuatVatTu()
  
      // Then load ChiTietCungUng data
      const chiTietResponse = await get(`/ChiTietCungUng/phieucungung/${supply.id}`)
      ...
      if (chiTietResponse.data && chiTietResponse.data.length > 0) {
        ...
        const transformedData = chiTietResponse.data.map(item => ({
          ...
          ghiChu: item.ghiChu,
          deXuatVatTuId: item.deXuatVatTu?.id // Use the ID from the nested object
        }))
